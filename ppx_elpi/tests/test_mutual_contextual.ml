...
let elpi_constant_constructor_tctx_TEentry = "teentry"
let elpi_constant_constructor_tctx_TEentryc =
  Elpi.API.RawData.Constants.declare_global_symbol
    elpi_constant_constructor_tctx_TEentry
module Ctx_for_term =
          ...
          | App (elpi__20, elpi__21) ->
              ...
              (elpi__state,
                (Elpi.API.RawData.mkAppL elpi_constant_constructor_term_Appc
                   [elpi__24; elpi__25]), (List.concat [elpi__22; elpi__23]))
          | Tapp (elpi__26, elpi__27) ->
              ...
              let (elpi__state, elpi__31, elpi__29) =
                elpi_embed_ty ~depth:elpi__depth elpi__hyps elpi__constraints
                  elpi__state elpi__27 in
              (elpi__state,
                (Elpi.API.RawData.mkAppL elpi_constant_constructor_term_Tappc
                   [elpi__30; elpi__31]), (List.concat [elpi__28; elpi__29]))
        ...
        fun elpi__state ->
          ...
          | TIdx (elpi__57, elpi__58) ->
              let (elpi__state, elpi__61, elpi__59) =
                elpi_embed_ty ~depth:elpi__depth elpi__hyps elpi__constraints
                  elpi__state elpi__57 in
              let (elpi__state, elpi__62, elpi__60) =
                elpi_embed_term ~depth:elpi__depth elpi__hyps
                  elpi__constraints elpi__state elpi__58 in
              (elpi__state,
                (Elpi.API.RawData.mkAppL elpi_constant_constructor_ty_TIdxc
                   [elpi__61; elpi__62]), (List.concat [elpi__59; elpi__60]))
          | TAbs (elpi__63, elpi__64, elpi__65) ->
              ...
              let elpi__state =
                elpi_pop_tctx ~depth:(elpi__depth + 1) elpi__state
                  elpi__ctx_key in
              (elpi__state,
                (Elpi.API.RawData.mkAppL elpi_constant_constructor_ty_TAbsc
      ...
      fun elpi__constraints ->
        fun elpi__state ->
          ...
          | (elpi__96, TEentry (elpi__94, elpi__95)) ->
              ...
              (elpi__state,
                (Elpi.API.RawData.mkAppL
                   elpi_constant_constructor_tctx_TEentryc
                ...
                (if
                 ...
                 (let { Elpi.API.Conversion.entry = elpi__entry;
                    ...
                    Elpi.API.RawData.Constants.Map.find elpi__hd
                      elpi__dbl2ctx in
                  (elpi__state,
                    (Var (elpi_tctx_to_key ~depth:elpi__depth elpi__entry)),
                    [])))
            ...
            | Elpi.API.RawData.App (elpi__hd, elpi__x, elpi__xs) when
                ...
                (match elpi__xs with
                 | elpi__48::elpi__49::[] ->
                     ...
                     let elpi__ctx_entry =
                       (fun s -> fun b -> TEntry (s, b)) elpi__53 elpi__48 in
                     let elpi__ctx_key =
                       elpi_tctx_to_key ~depth:elpi__depth elpi__ctx_entry in
                     let elpi__ctx_entry =
                       {
                ...
                (match elpi__xs with
                 | elpi__73::elpi__74::[] ->
                     let (elpi__state, elpi__73, elpi__75) =
                       Elpi.API.PPX.readback_string ~depth:elpi__depth
                         elpi__hyps elpi__constraints elpi__state elpi__73 in
                     let (elpi__state, elpi__74, elpi__76) =
                       elpi_readback_ty ~depth:elpi__depth elpi__hyps
                         elpi__constraints elpi__state elpi__74 in
                     (elpi__state, (elpi__78, (Entry (elpi__73, elpi__74))),
                     ...
                     Elpi.API.Utils.type_error
                       ("Not enough arguments to constructor: " ^
                          (Elpi.API.RawData.Constants.show
                             elpi_constant_constructor_tctx_Entryc)))
            | Elpi.API.RawData.App (elpi__hd, elpi__x, elpi__xs) when
                elpi__hd == elpi_constant_constructor_tctx_TEentryc ->
                ...
                (match elpi__xs with
                 | elpi__79::elpi__80::[] ->
                     let (elpi__state, elpi__79, elpi__81) =
                       Elpi.API.PPX.readback_string ~depth:elpi__depth
                         elpi__hyps elpi__constraints elpi__state elpi__79 in
                     let (elpi__state, elpi__80, elpi__82) =
                       Elpi.Builtin.PPX.readback_bool ~depth:elpi__depth
                         elpi__hyps elpi__constraints elpi__state elpi__80 in
                     (elpi__state,
    ...
    pp_doc =
      (fun fmt ->
         fun () ->
           ...
           Elpi.API.PPX.Doc.constructor fmt ~ty:kind ~name:"tidx" ~doc:"TIdx"
             ~args:[Elpi.API.Conversion.TyName elpi_constant_type_ty;
                   Elpi.API.Conversion.TyName elpi_constant_type_term];
           Elpi.API.PPX.Doc.constructor fmt ~ty:kind ~name:"tabs" ~doc:"TAbs"
    ...
    fun h ->
      fun c ->
        fun s ->
          ...
          let (s, gls0) =
            Elpi.API.PPX.readback_context ~depth context_made_of_tctx ctx h c
              s in
...
let () = declaration := ((!declaration) @ [elpi_term; elpi_ty; elpi_tctx])
[@@@end]

open Elpi.API

let in_ctx_for_term : ctx_for_term Conversion.ctx_readback = in_ctx_for_term
