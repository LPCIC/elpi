...
.. elpi:: ../code/incompatible.elpi
   :assert:  odd X\d+  /\* suspended on X\d+ \*/ even X\d+  /\* suspended on X\d+ \*/

Both constraints are accepted, quietly, side by side. The remedy is given in
the next chapter (:doc:`../semantics/chr`).