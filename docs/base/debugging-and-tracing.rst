...
Code extension's trace browser (:doc:`getting-started`) displays them and
steps through them interactively, rather than as a flat log:

.. figure:: /_static/trace-browser.png
   :alt: The VS Code trace browser, with a column of cards on the left (one
         per traced step, each naming its rule and predicate) and, on the
         right, the successful attempt and subgoals of the step currently
         selected.
   :width: 90%
   :align: center
...
failure, through :stdlib:`std.debug-print`, so it is subject to the same
overriding as any other library hook (:doc:`standard-library`). :stdlib:`std.spy!`
...
kept quiet: ``:if "NAME"`` (:doc:`syntax/rule-attributes`) drops a rule at
