...
- Exceptions: `catch P E` and `raise E`.

### Exceptions

The `catch` built-in predicate is described by
```prolog
type catch prop -> prop -> prop.
catch P E :- P, E = true.
catch _ E :- E = fail.
```
What makes it special is the `raise` built-in
```prolog
type raise prop -> prop
```
Intuitively
```prolog
catch (P, raise T, Q) E.
...
```
but `E` is set to `T` despite all assignments being done by `P` are
dropped.

Raised propositions are expected to fail, for example an error message
could be represented by
```prolog
type fail-str string -> prop.
```
If the convention is followed, then
```prolog
catch P E, E
```
is equivalent to `P`:
- if `P` succeeds then `E = true`, hence `E` also succeeds, and consequently
  `catch P E, E` succeeds and he bindings generated by `P` are kept
- if `P` fails or `raise` an exception `catch P E` succeeds but
  `E = fail` or `E = fail-str "message"`, hence `E` fails, and consequently
  `catch P E, E` fails and the bindings made by `P` are lost (but for the
...
An higher level idiom is to guard a block of code against
a set of possible exceptions

```prolog
try P OK Handlers :- catch P Res, if Res OK (handle Res Handlers).
handle Exn [ exn Exn B | _] :- !, B.
handle Exn [ _ | Handlers ] :- handle Exn Handlers.
