...
let (@:=) r v =
  ...
  if r.rest <> [] then begin
    [%spy "user:assign(resume)" (fun fmt l ->
...
let embed_query_args ctx ~mk_Arg =
  let rec aux : type a. depth:int -> predicate:constant -> term list -> term list -> constraints -> State.t -> (a,'ctx) Query.arguments -> State.t * term
  = fun ~depth ~predicate gls args constraints state descr ->
    match descr with
    | Data.Query.D(d,x,rest) ->
        let state, x, glsx = d.Conversion.embed ~depth ctx constraints state x in
        aux ~depth ~predicate (gls @ glsx) (x :: args) constraints state rest
    | Data.Query.Q(d,name,rest) ->
        let state, x = mk_Arg state ~name ~args:[] in
        aux ~depth ~predicate gls (x :: args) constraints state rest
    | Data.Query.N ->
...
let rec query_solution_aux : type a. (a,'ctx) Query.arguments -> term StrMap.t -> 'ctx -> constraints -> State.t -> a
 = fun args assignments ctx constraints state ->
     match args with
     | Data.Query.N -> ()
     | Data.Query.D(_,_,args) -> query_solution_aux args assignments ctx constraints state
     | Data.Query.Q(d,name,args) ->
         let x = StrMap.find name assignments in
         let state, x, _gls = d.Conversion.readback ~depth:0 ctx constraints state x in
         x, query_solution_aux args assignments ctx constraints state

let output ctx arguments assignments constraints state =
  query_solution_aux arguments assignments ctx constraints state

let embed_query ~mk_Arg ~depth hyps constraints state (Query.Query ({ Query.context; predicate; arguments },cc,builder)) =
  let ctx_builder = Data.Conversion.context_builder cc builder (new Conversion.ctx hyps) in
  let state, ctx_entries, ctx, gls = ctx_builder ~depth constraints context state in
  embed_query_args ctx ~mk_Arg ~depth ~predicate gls [] constraints state arguments,
  output ctx arguments
...
let make_subgoal_id ogid ((depth,goal)[@trace]) =
  ...
  [%spy "user:subgoalof" ~gid: ogid UUID.pp gid];
  [%spy "user:newgoal" ~gid (uppterm depth [] depth empty_env) goal];
  gid
  [@@inline]
...
let make_constraint_def ~gid:(gid[@trace]) depth prog pdiff conclusion =
...
let delay_goal ?(filter_ctx=fun _ -> true) ~depth prog ~goal:g (gid[@trace]) ~on:keys =
  ...
  [%spy "user:suspend" ~gid (uppterm depth [] 0 empty_env) g];
      ...
      [%spy "dev:CHR:candidate"
        (pplist (fun f x ->
           ...
           Format.fprintf f "(lives-at:%d, to-be-lifted-to:%d) %a"
             dt dto (uppterm dt [] 0 empty_env) t) ";") constraints_goals];
      ...
      | { depth = c_depth; mode = c_mode; args = c_args; hyps = c_hyps; vars = c_vars; loc } :: cs ->
        ...
        | true ->
           ...
           | h::hs ->
              ...
              let hs =
                List.map (fun x->
                  (make_subgoal[@inlined]) (gid[@trace]) ~depth p (move ~adepth:depth ~from:c_depth ~to_:depth env x))
                  hs in
  ...
  and cut (gid[@trace]) gs next alts lvl =
    ...
    let rec prune ({ agid = agid[@trace]; clauses; adepth = depth; agoal_hd = hd } as alts) =
      if alts == lvl then alts
        ...
        [%spy "user:cut" ~gid: agid (pplist (ppclause ~depth ~hd) " | ") clauses];
        prune alts.next
      end in
...
let mk_outcome search get_cs assignments =
 try
   let alts = search () in
   let syn_csts, state, readback_output, pp_ctx = get_cs () in
   let solution = {
     assignments;
     constraints = syn_csts;
     state;
     output = readback_output assignments syn_csts state;
     pp_ctx = pp_ctx;
   } in
...
let execute_once ?max_steps ?delay_outside_fragment exec =
 ...
 fst (mk_outcome search (fun () -> get CS.Ugly.delayed, get CS.state |> State.end_execution, exec.query_readback, { Data.uv_names = ref (get Pp.uv_names); table = get C.table }) exec.assignments)
;;
...
let execute_loop ?delay_outside_fragment exec ~more ~pp =
 ...
 let do_with_infos f =
  ...
  let o, alts = mk_outcome f (fun () -> get CS.Ugly.delayed, get CS.state |> State.end_execution, exec.query_readback, { Data.uv_names = ref (get Pp.uv_names); table = get C.table }) exec.assignments in
