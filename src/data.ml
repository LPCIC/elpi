...
let cons head tail =
  match tail with
  | BNil -> let rec last = BCons { head; tail; last } in last
  | BCons { last } -> BCons { head; tail; last }

let set_last_tail l = function
 | BCons x -> assert(x.tail = BNil);
 (* Format.eprintf "set tail of %d to %d \n" (Obj.magic w) (Obj.magic l);  *)
 x.tail <- l
 | BNil -> anomaly "blist: can't rcons after commit"

let rcons head tail =
  match tail with
  | BNil -> let rec last = BCons { head; tail = BNil; last } in last
  | BCons b -> let rec last = BCons { head; tail = BNil; last } in
      let olast = b.last in
      b.last <- last;
...
let commit l =
  ...
  | BNil -> ()
  | BCons ({ last; tail } as x) -> x.last <- BNil; commit tail
  in
  commit l;*) Obj.magic l
...
| TwoLevelIndex of {
    ...
    all_clauses : 'clause Bl.t;        (* when the query is flexible *)
    flex_arg_clauses : 'clause Bl.t;   (* when the query is rigid but arg_id ha nothing *)
    arg_idx : 'clause Bl.t Ptmap.t;    (* when the query is rigid (includes in each binding flex_arg_clauses) *)
  }
| BitHash of {
    mode : mode;
    args : int list;
