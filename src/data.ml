...
let rec show_ty_ast ?(outer=true) = function
  ...
  type ('a,'ctx) context_builder =
    depth:int -> constraints -> 'a list -> State.t ->
  ...
  let context_builder { conv; to_key; push; init } obj_builder hyps : ('a,'h) context_builder =
    ...
    fun ~depth csts items st ->
      let st = init st in
      let st, m, glsl_rev, _ =
        List.fold_left (fun (st, m, gls, depth) a ->
...
type ('stateful_builder,'builder, 'stateful_matcher, 'matcher,  'self, 'ctx) constructor_arguments =
  ...
  | C : (('self,Conversion.ctx) Conversion.t -> ('a,Conversion.ctx) Conversion.t) * ('bs,'b,'ms,'m,'self, 'ctx) constructor_arguments -> ('a -> 'bs, 'a -> 'b, 'a -> 'ms,'a -> 'm, 'self, 'ctx) constructor_arguments
...
let rec readback_args : type a m t.
  ...
  Conversion.ty_ast -> depth:int -> #Conversion.ctx -> constraints -> State.t -> extra_goals list -> term ->
  (a,m,t,Conversion.ctx) compiled_constructor_arguments -> a -> term list ->
...
and readback : type t.
  ...
  Conversion.ty_ast -> (t,Conversion.ctx) compiled_adt -> depth:int -> #Conversion.ctx -> constraints -> State.t -> term ->
    State.t * t * extra_goals
...
and adt_embed_args : type m a t.
  mkConst:(int -> term) ->
  Conversion.ty_ast -> (t,Conversion.ctx) compiled_adt -> constant ->
  depth:int -> #Conversion.ctx -> constraints ->
  (a,m,t,Conversion.ctx) compiled_constructor_arguments ->
...
and embed : type a.
  ...
  depth:int -> #Conversion.ctx -> constraints -> State.t ->
...
let rec compile_arguments : type b bs m ms t.
  (bs,b,ms,m,t,Conversion.ctx) constructor_arguments -> (t,#Conversion.ctx) Conversion.t -> (bs,ms,t,Conversion.ctx) compiled_constructor_arguments =
fun arg self ->
  match arg with
  | N -> XN
