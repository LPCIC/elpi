...
(* A discrimination tree is a record {t; max_size; max_depths } where
  ...
  *)
type 'a t = {t: 'a data Trie.t; max_size: int;  max_depths: int array; max_list_length: int } 
...
let index { t; max_size; max_depths; max_list_length = mll } ~max_list_length path data ~time =
