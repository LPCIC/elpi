...
let rec to_heap argsdepth ~from ~to_ ?(avoid=def_avoid) e t =
  ...
  let rec aux depth x =
    ...
    | UVar ({ contents = t },vardepth,args) when t != dummy ->
       if depth == 0 then
         do_full_deref argsdepth ~from:vardepth ~to_ args e t
       else
         (* First phase: from vardepth to from+depth *)
         let t =
            do_full_deref argsdepth 
...
and do_full_deref argsd ~from ~to_ ?f args e t =
  match args with
  | Noa -> full_deref argsd ~from ~to_ 0 e t
  | Irl n -> full_deref argsd ~from ~to_ n e t
  | Exp tl ->
