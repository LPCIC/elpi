...
module PointerFunc = struct
 let flag = ref false
 let export_clause_pointer = ref (fun _ -> assert false)
 let export_header_pointer = ref ""
 let export_footer_pointer = ref ""
 let set_export_clause f = export_clause_pointer := f
 let set_export_header s = export_header_pointer := s 
 let set_export_footer s = export_footer_pointer := s 
...
let rec parse_one e (origfilename as filename) =
 ...
 if List.mem inode !parsed then begin
  ...
  sigs @
  try
   let old_open_tex_file = !PointerFunc.open_tex_file in
   if !PointerFunc.flag then begin
    ...
    Printf.fprintf ch_out_tex "%s\n%s\n\n%!" (procent_str ^" This file is automatically generated from the file:") (procent_str ^ " " ^ filename);     
    Printf.fprintf ch_out_tex "%s%!" (!PointerFunc.export_header_pointer);
    PointerFunc.open_tex_file := ch_out_tex
   end;
   let res = Grammar.Entry.parse e (Stream.of_channel ch) in
   if !PointerFunc.flag then begin
    Printf.fprintf (!PointerFunc.open_tex_file) "%s%!" (!PointerFunc.export_footer_pointer);
    close_out !PointerFunc.open_tex_file;
    PointerFunc.open_tex_file := old_open_tex_file
   end;
   close_in ch;
