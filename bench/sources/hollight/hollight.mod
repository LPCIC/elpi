...
thm A X :- is_flex X, saved A X, !, thm A X.

thm G C :- $print "|- " G " := " C, fail.
thm G C :- is_flex C, !, $delay (thm G C) C.
thm G C :- thm_ G C.

thm_ G C :- $print "||- " G " := " C, fail.
...
$print "XXX" X A,
 term X A, $print "ZZZ" Y A, term Y A,
$print "YYY",
 (pi y\ term y A => term (F ' y) B),
...
saved (eq ' (eq ' (lam x\ x) ' (lam x\ x)) ' tt) (i (eq ' tt ' tt) (c r (c d r)) r).


/*
thm (eq ' (eq ' (lam x\ x) ' (lam x\ x)) ' tt) (i (eq ' tt ' tt) (c r (c d r)) r).
*/

/*
pi x\ pi y\ pi a\ pi b\ term x a => term y b =>
 thm (eq (eq ' x ' y) (eq ' y ' x)) (C x y)
*/

/*thm (eq (eq ' X ' Y) (eq ' Y ' X)) ax.
thm tt (i (eq ' (lam x\ x) ' (lam x\ x)) _ r).*/
