...
lam F !!! C /*A --> B*/ :- pi x\ x !!! A => (F x !! B, $print "C = A --> B  input  " (C = A --> B), C = A --> B, $print "C = A --> B  output " (C = A --> B)).
...
letrec (f \ [BO f,REST f]) !!! B :-
 (pi f\ (pi X \ f !!! A X) => ((pi x\ (BO f !! (R x), $print "R" x " = " (R x), $print "R = " R, $print (R x) "===" (A x), R x = A x, $print A)), REST f !! B)),
 $print f ":" A.
